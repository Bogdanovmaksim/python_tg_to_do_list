...
class AddTaskStates(StatesGroup):
    """
    Группа состояний для процесса добавления задачи.

    Состояния:
        waiting_for_text: ожидание ввода текста задачи
        waiting_for_category: ожидание ввода категории (опционально)
        waiting_for_deadline: ожидание ввода дедлайна (опционально)
    """
...
def get_choice_keyboard(yes_text: str, no_text: str, yes_callback: str, no_callback: str):
    """
...
async def finalize_add_task(source, state: FSMContext):
    ...
    logging.info(
        f"finalize_add_task: user={user_id} "
        f"text={task_text!r} category={category!r} deadline={deadline!r}"
    )
...
async def cmd_list_callback(callback_query: types.CallbackQuery):
    """
    Функция обработчик кнопки "Список задач".

    Выводит пользователю все активные и завершённые задачи.
...
async def show_statistics(callback_query: types.CallbackQuery):
    """
    Функция обрабатывает кнопку "Статистика".

    Выводит пользователю полную статистику по его задачам.
...
async def unknown_command(message: Message):
    ...
    :param message: сообщение с неизвестной командой
    :type message: aiogram.types.Message
    :returns: None
    """
    await message.reply(
...
async def main():
    ...
    Запускает планировщик напоминаний и начинает поллинг бота.
